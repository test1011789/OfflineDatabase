...
class Database:
    ...
    def create_production_record(self, record_id: int, production_date: str = '', batch_no: str = '', quantity: str = '', remark: str = '') -> int:
        if not self.conn.execute('SELECT 1 FROM records WHERE id = ?', (int(record_id),)).fetchone():
            raise ValueError('找不到對應的主資料。')
        now = datetime.now().isoformat(timespec='seconds')
        cursor = self.conn.execute(
            '''INSERT INTO production_records(record_id, production_date, batch_no, quantity, remark, created_at, updated_at)
               VALUES (?, ?, ?, ?, ?, ?, ?)''',
            (int(record_id), production_date.strip(), batch_no.strip(), quantity.strip(), remark.strip(), now, now),
        )
        self.conn.commit()
        return int(cursor.lastrowid)

    def update_production_record(self, production_id: int, production_date: str, batch_no: str, quantity: str, remark: str) -> None:
        self.conn.execute(
            '''UPDATE production_records
               SET production_date = ?, batch_no = ?, quantity = ?, remark = ?, updated_at = ?
               WHERE id = ?''',
            (production_date.strip(), batch_no.strip(), quantity.strip(), remark.strip(), datetime.now().isoformat(timespec='seconds'), int(production_id)),
        )
        self.conn.commit()
...
class OfflineDatabaseApp(tk.Tk):
    ...
    def open_production_records(self, record_id: int) -> None:
        ...
        header = ttk.Frame(win, padding=(18, 14, 18, 8))
        header.pack(fill='x')
        ttk.Label(header, text='生產記錄', style='Title.TLabel').pack(side='left')
        ttk.Label(header, text=self._record_display_name(record_id), style='Hint.TLabel').pack(side='left', padx=18)

        frame = ttk.Frame(win, padding=(18, 4, 18, 8))
        frame.pack(fill='both', expand=True)
        columns = ('date', 'batch', 'quantity', 'remark')
        tree = ttk.Treeview(frame, columns=columns, show='headings', selectmode='browse')
        headings = [('date', '生產日期', 130), ('batch', '批號', 180), ('quantity', '生產數量', 130), ('remark', '備註', 360)]
        for key, text, width in headings:
            tree.heading(key, text=text, anchor='center')
            tree.column(key, width=width, anchor='center' if key != 'remark' else 'w', stretch=(key == 'remark'))
        ...
        def refresh() -> None:
            ...
            for row in self.db.production_records(record_id):
                tree.insert('', 'end', iid=str(row['id']), values=(row['production_date'], row['batch_no'], row['quantity'], row['remark']))
            self.refresh_data()
        ...
        def edit(existing_id: int | None = None) -> None:
            old = None
            ...
            vars_ = {
                ...
                'batch': tk.StringVar(value='' if old is None else old['batch_no']),
                'quantity': tk.StringVar(value='' if old is None else old['quantity']),
                'remark': tk.StringVar(value='' if old is None else old['remark']),
            }
            body = ttk.Frame(dialog, padding=18)
            body.grid(sticky='nsew')
            labels = [('date', '生產日期'), ('batch', '批號'), ('quantity', '生產數量'), ('remark', '備註')]
            for row_index, (key, label) in enumerate(labels):
                ttk.Label(body, text=label, width=12).grid(row=row_index, column=0, sticky='w', padx=(0, 10), pady=6)
                ttk.Entry(body, textvariable=vars_[key], width=48).grid(row=row_index, column=1, sticky='ew', pady=6)
            ...
            def save() -> None:
                try:
                    if old is None:
                        self.db.create_production_record(record_id, vars_['date'].get(), vars_['batch'].get(), vars_['quantity'].get(), vars_['remark'].get())
                    else:
                        self.db.update_production_record(existing_id, vars_['date'].get(), vars_['batch'].get(), vars_['quantity'].get(), vars_['remark'].get())
                    dialog.destroy()
                    refresh()
                except Exception as exc:
        ...
        def delete_selected() -> None:
            ...
            if messagebox.askyesno('刪除生產記錄', '確定要刪除選取的生產記錄嗎？', parent=win):
                self.db.delete_production_record(production_id)
                refresh()

        toolbar = ttk.Frame(win, padding=(18, 4, 18, 16))
        toolbar.pack(fill='x')
        ttk.Button(toolbar, text='新增', command=lambda: edit(None)).pack(side='left')
        ttk.Button(toolbar, text='編輯', command=edit_selected).pack(side='left', padx=6)
        ttk.Button(toolbar, text='刪除', command=delete_selected).pack(side='left')
        ttk.Button(toolbar, text='關閉', command=win.destroy).pack(side='right')
